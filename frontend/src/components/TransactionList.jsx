...
export default function TransactionList({
  ...
  return (
    <ul
      className="divide-y divide-gray-800 rounded-xl border border-gray-800 overflow-hidden"
      role="list"
    >
      {transactions.map((txn, idx) => {
        // Determine direction relative to this user's accounts
        const isCredit = userAccountIds.has(
          String(txn.toAccount?._id ?? txn.toAccount),
        );
          ...
          >
            ...
            <div className="shrink-0 text-right">
              ...
              <span
                className={clsx(
                  "mt-0.5 inline-block",
                  STATUS_CLS[txn.status] ?? "badge-yellow",
                )}
              >
                {txn.status}
              </span>
